...
#Create a plot window with four frames
par(mfrow=c(2,2))

#Plot graph 1
plot(newtime,p1[,3],type="l",xlab="",ylab="Global Active Power (kilowatts)")
#Plot graph 2
plot(newtime,p1[,5],type="l", xlab="datetime",ylab="Voltage")
#Plot graph 3
plot(newtime,p1[,7],type="l",xlab="",ylab="Energy sub metering",col=c("black"))
with(p1,lines(newtime,p1[,8],col="red", type="l"))
with(p1,lines(newtime,p1[,9],col="blue", type="l"))
legend("topright", pch =1, col = c("blue","red"), legend = c("Sub_metering_1","Sub_metering_2","Sub_metering_3"))
#Plot graph 4
plot(newtime,p1[,4],type="l",xlab="datetime",ylab="Global_reactive_power")
